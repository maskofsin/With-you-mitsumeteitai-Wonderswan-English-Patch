...
PAIR_LOOKUP = MAPPING
SPECIAL_BY_PREFIX = {}
for tok, code in SPECIAL_TOKENS:
    SPECIAL_BY_PREFIX.setdefault(tok[0], []).append((tok, code))
for key in SPECIAL_BY_PREFIX:
    SPECIAL_BY_PREFIX[key].sort(key=lambda item: len(item[0]), reverse=True)
...
def try_decode_shift_jis(data: bytes) -> str:
    try:
...
def hex_to_bytes(code_hex: str) -> bytes:
    compact = re.sub(r"[^0-9A-Fa-f]", "", code_hex)
    if len(compact) % 2:
        compact += "0"
    return bytes.fromhex(compact)


def bytes_to_hex_spaced(blob: bytes) -> str:
    return " ".join(f"{b:02X}" for b in blob)


def code_string_to_bytes(code_hex: str) -> bytes:
    chunks = code_hex.split()
    if not chunks:
        return b""
    out = bytearray()
    for chunk in chunks:
        ...
        if not re.fullmatch(r"[0-9A-Fa-f]{4}", c):
            raise ValueError(f"Invalid code: {chunk}")
        out.extend(bytes.fromhex(c))
    return bytes(out)


def encode_regular_chunk(chunk: str, auto_pad: bool = True):
    tokens = []
    codes = []
    script_bytes = []
    warnings = []
    ...
    if not chunk:
        return tokens, codes, script_bytes, warnings
    ...
    if len(segment) % 2 == 1:
        if auto_pad:
            ...
            warnings.append("Odd-length regular segment without padding.")

    for i in range(0, len(segment), 2):
        tok = segment[i:i + 2]
        code = PAIR_LOOKUP.get(tok)
        tokens.append(tok)
        if code is None:
            codes.append("????")
            script_bytes.append("?? ??")
            warnings.append(f"Unsupported pair: {visible_token(tok)}")
        else:
            codes.append(code)
            script_bytes.append(f"{code[:2]} {code[2:]}")
    return tokens, codes, script_bytes, warnings


def encode_translation(text: str, auto_pad: bool = True):
    """
    Greedy scan:
    - Special punctuation tokens are emitted as standalone codes.
    - Everything else is encoded with the existing bigram map.
    """
    tokens = []
    codes = []
    script_bytes = []
    ...
    while i < len(text):
        matched = None
        ch = text[i]
        if ch in SPECIAL_BY_PREFIX:
            for tok, code in SPECIAL_BY_PREFIX[ch]:
        ...
        if matched:
            if buffer:
                btoks, bcodes, bbytes, bwarns = encode_regular_chunk("".join(buffer), auto_pad=auto_pad)
                tokens.extend(btoks)
                codes.extend(bcodes)
                script_bytes.extend(bbytes)
...
def pad_bytes_to_length(encoded: bytes, length: int, pad_pattern: bytes) -> bytes:
    if length <= 0:
        return encoded
    ...
    if not pad_pattern:
        pad_pattern = b"\x00"
    need = length - len(encoded)
    repeated = (pad_pattern * (need // len(pad_pattern) + 1))[:need]
    return encoded + repeated


def read_table_file(path: str):
    """
    Supported formats:
    1) Headered CSV/TSV with columns like:
       offset_hex, length_hex, raw_hex, decoded, visible, controls, unknown
       or line, offset, length, jp_text, translation, note
    2) Simple delimited rows:
       line<TAB>offset<TAB>length<TAB>jp_text<TAB>translation(optional)
    """
    ...
    header_tokens = [s.strip().lower() for s in sample.split(delim)]
    has_header = any(
        token in header_tokens
        for token in [
            "line", "index", "offset", "offset_hex", "length", "length_hex",
            "jp_text", "decoded", "translation", "visible", "controls", "unknown", "raw_hex"
        ]
    )
    ...
    if has_header:
        ...
        for row_index, row in enumerate(reader, start=1):
            ...
            line_raw = first_value(row, ["line", "index", "line_no", "row"], "")
            if line_raw:
                line_no = parse_number(line_raw)
            else:
                line_no = row_index

            offset = parse_number(first_value(row, ["offset_hex", "offset", "addr", "address", "ofs"], "0"))
            length = parse_number(first_value(row, ["length_hex", "length", "size", "len"], "0"))

            # Prefer decoded text as the source/original JP line.
            jp_text = first_value(
                row,
                ["decoded", "jp_text", "jp", "original", "source", "text", "visible"],
                ""
            )
            translation = first_value(
                row,
                ["translation", "en_text", "english", "target", "translated"],
                ""
            )

            raw_hex = first_value(row, ["raw_hex"], "")
            visible = first_value(row, ["visible"], "")
            controls = first_value(row, ["controls"], "")
            unknown = first_value(row, ["unknown"], "")

            # Keep useful extra fields in the note column for display.
            extras = []
            for label, value in [("visible", visible), ("controls", controls), ("unknown", unknown)]:
                if value:
                    extras.append(f"{label}={value}")
            note = first_value(row, ["note", "remarks"], "")
            if extras:
                note = (note + " | " if note else "") + " | ".join(extras)

            entries.append(ScriptEntry(line_no, offset, length, jp_text, translation, raw_hex, note))
    else:
        for row_index, row in enumerate(csv.reader(raw.splitlines(), delimiter=delim), start=1):
            if not row or not any(cell.strip() for cell in row):
                continue
            while len(row) < 5:
                row.append("")
            line_raw = row[0].strip()
            line_no = parse_number(line_raw) if line_raw else row_index
            offset = parse_number(row[1])
            length = parse_number(row[2])
            jp_text = row[3].strip()
            translation = row[4].strip()
            raw_hex = row[5].strip() if len(row) > 5 else ""
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("With You ROM Patch Assistant")
        ...
        self.auto_pad_var = tk.BooleanVar(value=True)
        self.auto_encode_var = tk.BooleanVar(value=True)
        self.auto_save_var = tk.BooleanVar(value=False)
        self.pad_hex_var = tk.StringVar(value="8140")
        self.status_var = tk.StringVar(value="Open a ROM and load a script table.")

        self._build_ui()
        self.bind_all("<Control-Return>", lambda e: self.encode_current())
        self.bind_all("<Control-s>", lambda e: self.save_rom())
        self.bind_all("<Control-o>", lambda e: self.open_rom())
        self.bind_all("<Control-l>", lambda e: self.load_table())
        self.bind_all("<Control-Right>", lambda e: self.next_entry())
        self.bind_all("<Control-Left>", lambda e: self.prev_entry())

    def _build_ui(self):
        top = ttk.Frame(self, padding=8)
        top.pack(fill="x")

        ttk.Label(top, text="ROM").grid(row=0, column=0, sticky="w")
        self.rom_var = tk.StringVar(value="")
        rom_entry = ttk.Entry(top, textvariable=self.rom_var)
        rom_entry.grid(row=0, column=1, sticky="ew", padx=(8, 8))
        ttk.Button(top, text="Open ROM", command=self.open_rom).grid(row=0, column=2, padx=(0, 6))
        ttk.Button(top, text="Save ROM", command=self.save_rom).grid(row=0, column=3, padx=(0, 6))
        ttk.Button(top, text="Save As", command=self.save_rom_as).grid(row=0, column=4, padx=(0, 6))
        ttk.Button(top, text="Load Table", command=self.load_table).grid(row=0, column=5, padx=(0, 6))

        top.columnconfigure(1, weight=1)

        opts = ttk.Frame(self, padding=(8, 0, 8, 4))
        opts.pack(fill="x")
        ttk.Checkbutton(opts, text="Auto-pad odd regular segments", variable=self.auto_pad_var, command=self.encode_current).pack(side="left")
        ttk.Checkbutton(opts, text="Auto-encode while typing", variable=self.auto_encode_var).pack(side="left", padx=(16, 0))
        ttk.Checkbutton(opts, text="Auto-save after patch", variable=self.auto_save_var).pack(side="left", padx=(16, 0))
        ttk.Label(opts, text="Pad hex").pack(side="left", padx=(24, 4))
        ttk.Entry(opts, textvariable=self.pad_hex_var, width=10).pack(side="left")
        ttk.Button(opts, text="Encode", command=self.encode_current).pack(side="left", padx=(16, 0))
        ttk.Button(opts, text="Patch Selected Line", command=self.patch_selected_line).pack(side="left", padx=(8, 0))
        ...
        # Left side: line list and selected line details.
        list_frame = ttk.LabelFrame(left, text="Script lines", padding=6)
        list_frame.pack(fill="both", expand=True)

        toolbar = ttk.Frame(list_frame)
        toolbar.pack(fill="x", pady=(0, 6))
        ttk.Button(toolbar, text="Prev", command=self.prev_entry).pack(side="left")
        ttk.Button(toolbar, text="Next", command=self.next_entry).pack(side="left", padx=(6, 0))
        ttk.Label(toolbar, text="Jump to line").pack(side="left", padx=(16, 4))
        self.jump_var = tk.StringVar(value="")
        ttk.Entry(toolbar, textvariable=self.jump_var, width=10).pack(side="left")
        ttk.Button(toolbar, text="Go", command=self.jump_to_line).pack(side="left", padx=(6, 0))

        self.entry_list = tk.Listbox(list_frame, height=16, activestyle="dotbox")
        ...
        detail = ttk.LabelFrame(left, text="Selected line", padding=8)
        detail.pack(fill="x", pady=(8, 0))

        grid = ttk.Frame(detail)
        grid.pack(fill="x")
        ttk.Label(grid, text="Line").grid(row=0, column=0, sticky="w")
        self.line_var = tk.StringVar(value="-")
        ttk.Label(grid, textvariable=self.line_var).grid(row=0, column=1, sticky="w", padx=(6, 18))
        ttk.Label(grid, text="Offset").grid(row=0, column=2, sticky="w")
        self.offset_var = tk.StringVar(value="-")
        ttk.Label(grid, textvariable=self.offset_var).grid(row=0, column=3, sticky="w", padx=(6, 18))
        ttk.Label(grid, text="Length").grid(row=0, column=4, sticky="w")
        self.length_var = tk.StringVar(value="-")
        ttk.Label(grid, textvariable=self.length_var).grid(row=0, column=5, sticky="w")

        ttk.Label(detail, text="Raw Hex").pack(anchor="w", pady=(8, 2))
        self.raw_hex_text = tk.Text(detail, height=3, wrap="word", font=("Consolas", 11))
        self.raw_hex_text.pack(fill="x")
        self.raw_hex_text.configure(state="disabled")

        ttk.Label(detail, text="Original Japanese").pack(anchor="w", pady=(8, 2))
        self.jp_text = tk.Text(detail, height=5, wrap="word", font=("Consolas", 11))
        self.jp_text.pack(fill="x")
        self.jp_text.configure(state="disabled")

        ttk.Label(detail, text="Translation").pack(anchor="w", pady=(8, 2))
        self.translation_var = tk.StringVar(value="")
        self.translation_entry = ttk.Entry(detail, textvariable=self.translation_var)
        self.translation_entry.pack(fill="x")
        self.translation_entry.bind("<KeyRelease>", self._on_translation_edit)

        buttons = ttk.Frame(detail)
        buttons.pack(fill="x", pady=(8, 0))
        ttk.Button(buttons, text="Copy raw hex", command=lambda: self.copy_text(self.raw_hex_text)).pack(side="left")
        ttk.Button(buttons, text="Copy hex code", command=lambda: self.copy_text(self.code_hex_text)).pack(side="left", padx=(6, 0))
        ttk.Button(buttons, text="Copy script bytes", command=lambda: self.copy_text(self.script_bytes_text)).pack(side="left", padx=(6, 0))
        ttk.Button(buttons, text="Copy token pairs", command=lambda: self.copy_text(self.tokens_text)).pack(side="left", padx=(6, 0))

        # Right side: encoder preview and warnings.
        summary = ttk.LabelFrame(right, text="Summary", padding=8)
        summary.pack(fill="x")
        self.summary_text = tk.Text(summary, height=5, wrap="word", font=("Consolas", 10))
        ...
        self.tokens_text = self._make_readonly_tab(notebook, "Token pairs")
        self.code_hex_text = self._make_readonly_tab(notebook, "Code hex")
        self.script_bytes_text = self._make_readonly_tab(notebook, "Script bytes")
        self.warnings_text = self._make_readonly_tab(notebook, "Warnings")

        status = ttk.Label(self, textvariable=self.status_var, anchor="w", padding=(8, 0, 8, 8))
        status.pack(fill="x")

    def _make_readonly_tab(self, notebook, title):
    ...
    def set_readonly_text(self, widget, value: str):
    ...
    def open_rom(self):
        path = filedialog.askopenfilename(
            title="Open ROM",
            filetypes=[("ROM files", "*.rom *.wsc *.bin *.ws *.dat *.*"), ("All files", "*.*")]
        )
        if not path:
            return
        try:
            ...
            messagebox.showerror("Open ROM", str(e))
            return
        self.rom_path = path
    ...
    def save_rom(self):
        if not self.rom_path:
            messagebox.showinfo("Save ROM", "Open a ROM first.")
            return
        try:
            with open(self.rom_path, "wb") as f:
                f.write(self.rom_data)
        except OSError as e:
            messagebox.showerror("Save ROM", str(e))
            return
        self.dirty = False
        self.status_var.set(f"Saved ROM: {self.rom_path}")

    def save_rom_as(self):
        if not self.rom_data:
            messagebox.showinfo("Save As", "Open a ROM first.")
            return
        ...
        self.rom_var.set(path)
        self.dirty = False
        self.status_var.set(f"Saved ROM As: {path}")

    def load_table(self):
        path = filedialog.askopenfilename(
            title="Load script table",
            filetypes=[("CSV/TSV files", "*.csv *.tsv *.txt"), ("All files", "*.*")]
        )
        if not path:
            return
        try:
            self.entries = read_table_file(path)
        except Exception as e:
            messagebox.showerror("Load Table", str(e))
            return
        self.entry_list.delete(0, "end")
        for entry in self.entries:
            preview = entry.jp_text or "(no JP text)"
            preview = preview.replace("\n", " ")
            if len(preview) > 24:
                preview = preview[:24] + "…"
            raw_preview = entry.raw_hex or ""
            if len(raw_preview) > 12:
                raw_preview = raw_preview[:12] + "…"
            self.entry_list.insert("end", f"{entry.line_no:>4} | {entry.offset:08X} | {entry.length:>4} | {raw_preview:<13} | {preview}")
        self.current_index = 0 if self.entries else None
        if self.entries:
            self.entry_list.selection_set(0)
            self.entry_list.see(0)
        self.refresh_current_entry()
        self.status_var.set(f"Loaded {len(self.entries)} table rows from {path}")
    ...
    def jump_to_line(self):
        ...
        if not target:
            return
        try:
            line_no = parse_number(target)
        except ValueError as e:
            messagebox.showerror("Jump to line", str(e))
            return
        for idx, entry in enumerate(self.entries):
            if entry.line_no == line_no:
                ...
                self.refresh_current_entry()
                return
        messagebox.showinfo("Jump to line", f"Line {line_no} was not found.")
    ...
    def prev_entry(self):
        ...
        self.entry_list.selection_clear(0, "end")
        self.entry_list.selection_set(self.current_index)
        self.entry_list.see(self.current_index)
        self.refresh_current_entry()
    ...
    def refresh_current_entry(self):
        ...
        if entry is None:
            ...
            self.set_readonly_text(self.jp_text, "")
            self.set_readonly_text(self.summary_text, "No line selected.")
            self.clear_outputs()
            return

        self.line_var.set(str(entry.line_no))
        self.offset_var.set(f"0x{entry.offset:X}")
        self.length_var.set(str(entry.length))

        original = entry.jp_text.strip()
        if not original and self.rom_data and entry.offset < len(self.rom_data):
            end = min(len(self.rom_data), entry.offset + entry.length)
            original = try_decode_shift_jis(bytes(self.rom_data[entry.offset:end]))
        self.set_readonly_text(self.jp_text, original or "(no original text)")

        self.translation_var.set(entry.translation or "")
        self.set_readonly_text(self.raw_hex_text, entry.raw_hex or "(no raw hex)")
        self.encode_current()

    def _on_translation_edit(self, event=None):
        if self.auto_encode_var.get():
            self.encode_current()

    def clear_outputs(self):
        self.set_readonly_text(self.raw_hex_text, "")
        self.set_readonly_text(self.tokens_text, "")
        self.set_readonly_text(self.code_hex_text, "")
        self.set_readonly_text(self.script_bytes_text, "")
        self.set_readonly_text(self.warnings_text, "")
        self.set_readonly_text(self.summary_text, "")
    ...
    def encode_current(self):
        ...
        if text == "":
            ...
            if entry:
                summary = (
                    f"Selected line: {entry.line_no}\n"
                    f"Offset: 0x{entry.offset:X}\n"
                    f"Length: {entry.length}\n"
                    f"ROM loaded: {'yes' if self.rom_data else 'no'}"
                )
                self.set_readonly_text(self.summary_text, summary)
            return

        result = encode_translation(text, auto_pad=self.auto_pad_var.get())
        ...
        token_lines = " ".join(visible_token(t) for t in tokens)
        code_lines = " ".join(codes)
        bytes_lines = " ".join(script_bytes)
        warn_lines = "\n".join(warnings) if warnings else "No warnings."

        self.set_readonly_text(self.tokens_text, token_lines)
        self.set_readonly_text(self.code_hex_text, code_lines)
        self.set_readonly_text(self.script_bytes_text, bytes_lines)
        self.set_readonly_text(self.warnings_text, warn_lines)

        unsupported = sum(1 for c in codes if c == "????")
        entry = self.get_selected_entry()
        summary_parts = [
            f"Chars: {len(text)}",
            f"Tokens: {len(tokens)}",
            f"Unsupported pairs: {unsupported}",
    ...
    def patch_selected_line(self):
        entry = self.get_selected_entry()
        if entry is None:
            messagebox.showinfo("Patch Selected Line", "Select a line first.")
            return
        if not self.rom_data:
            messagebox.showinfo("Patch Selected Line", "Open a ROM first.")
            return

        text = self.translation_var.get()
        if text == "":
            messagebox.showinfo("Patch Selected Line", "Enter a translation first.")
            return

        result = encode_translation(text, auto_pad=self.auto_pad_var.get())
        if any(code == "????" for code in result["codes"]):
            messagebox.showerror("Patch Selected Line", "There are unsupported pairs in the translation.")
            return

        try:
            encoded = code_string_to_bytes(" ".join(result["codes"]))
        except ValueError as e:
            messagebox.showerror("Patch Selected Line", str(e))
            return

        pad_hex = self.pad_hex_var.get().strip() or "00"
        try:
            pad_pattern = hex_to_bytes(pad_hex)
        except ValueError as e:
            messagebox.showerror("Patch Selected Line", f"Invalid pad hex: {e}")
            return
        ...
        if entry.length > 0:
            if len(encoded) > entry.length:
                ok = messagebox.askyesno(
                    "Patch Selected Line",
                    f"Encoded data is {len(encoded)} bytes, but the line length is {entry.length}.\n"
                    f"Truncate the encoded bytes to fit?"
                )
                if not ok:
                    return
            patched = pad_bytes_to_length(encoded, entry.length, pad_pattern)
        else:
            patched = encoded

        end = entry.offset + len(patched)
        if entry.offset < 0 or end > len(self.rom_data):
            messagebox.showerror("Patch Selected Line", "This line falls outside the loaded ROM buffer.")
            return

        self.rom_data[entry.offset:end] = patched
        self.dirty = True
        self.status_var.set(f"Patched line {entry.line_no} directly into loaded ROM at 0x{entry.offset:X}")
